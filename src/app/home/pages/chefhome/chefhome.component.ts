...
import { Calendar, CustomerOrder, Food, LocalChef } from 'src/app/model/localchef';
...
export class ChefHomeComponent implements AfterViewInit {
  ...
  private fetchItems(chefId: string) {
    this.chefService.getAllFoods(chefId).subscribe((items: Food[]) => {
      ...
      var category = this.chef.categories[0];
  ...
  computeDays() {
    var weekDays: Day[] = []
    var theDate = new Date(), year = theDate.getFullYear(), month = theDate.getMonth();
    for (let i = 0; i < 7; i++) {
  ...
  filterItemsByCat(category: string) {

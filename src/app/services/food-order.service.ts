...
import { CustomerOrder, CustomerOrderList, FoodOrderItem, LocalChef, Orders, OrderSearchQuery, OrderUpdateRequest, SupplierOrder, SupplierOrders } from '../model/localchef';
...
export class FoodOrderservice {
  ...
  saveOrder(order: CustomerOrder): Observable<CustomerOrder> {
    console.log('Saving the order: '+ JSON.stringify(order))
    var url = this.HOST + this.ORDERS_URI;
    return this.http.post<CustomerOrder>(url, order);
  }

  retrieveOrder(orderId: string): Observable<CustomerOrder> {
    var url = this.HOST + this.ORDERS_URI + "/" + orderId;
  ...
  public calculateTotal(order:CustomerOrder) {
    ...
    order.serviceFee = 0.00;
    order.subTotal = subTotal;
    if ( subTotal !== 0){
      order.serviceFee = 0.50;
      if (order.serviceMode === 'DELIVERY'){
        order.deliveryFee = 0.50;
      }
    }
  
    var totalToPay:number = order.subTotal + order.deliveryFee + order.packingFee + order.serviceFee;
  ...
  createPaymentIntentForOrder(customerOrder: CustomerOrder): Observable<PaymentIntentResponse>{
    ...
    var url = this.HOST + this.STRIPE_URI + this.PAYMENT_INTENT;
    console.log('Creating payment intent: ' + url + ", " + JSON.stringify(paymentIntentRequest));
  ...
  placeOrder(customerOrder: CustomerOrder): Observable<CustomerOrder> {
    var url = this.HOST + this.ORDERS_URI;
    console.log('Placing an order for LocalChef : ' + url + ", " + JSON.stringify(customerOrder));
